...
void UABCharacterSelectWidget::NextCharacter(bool bForward) {
	...
	auto ABGameInstace = GetWorld()->GetGameInstance<UABGameInstance>();
	ABCHECK(ABGameInstace != nullptr);
	ABCHECK(TargetComponent.IsValid());

	USkeletalMesh* Asset = ABGameInstace->StreamableManager.LoadSynchronous<USkeletalMesh>(AssetRef);
	if (Asset != nullptr)
